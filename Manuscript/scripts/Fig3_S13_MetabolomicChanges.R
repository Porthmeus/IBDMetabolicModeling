...
coef.fls <- list.files(ddr, pattern = ".*LMM.*_coefs.csv.*$")
...
enriched <- fread(file.path(ddrHost, "EnrichedSubsystems.csv"))
...
#    st <- "HBMayo"
    ...
    plt.bar <- ggplot(dat.bar, aes(y = N,x = x, fill = Category2))+
        ...
        labs(x="", y = "No. of metabolites", fill = "Met. Class") +
    ...
    if(set.title != "HBI/Mayo score"){
        ...
        xlab <- "Estimate (level change during inflam.)"
    }
    ...
    plt.up <- plot_grid(NULL, estimate.plt,plt.bar, NULL, nrow = 1, rel_widths = c(0.4,1.3,1,0.4),labels = c(NA,"A","B",NA))
    plt.all <- plot_grid(plt.up,  sankey_plot, ncol = 1, labels = c(NA,"C"), rel_heights = c(1,1.5))
    ggsave(plt.all, file = file.path(ddrFig, paste0("Fig3_MetabolomicChanges.",st,".pdf")),
           height = 3*length(met.levels)*0.1 +2,
           width = 11)
