...
for(set in c("Response","Remission")){
    ...
    pl <- ggplot(coefs.drctn[padj <= 0.05,],aes(x=Estimate, y = name, color = dataset, shape = by.microbiome)) +
        ...
        ggtitle(title)+
        theme_bw()
    ggsave(pl, file = paste0(file.path("..","figures_raw","/FigSX"),LETTERS[figno],"_MicrobialMetabolites.", set, ".woCorrection.pdf"), height = 0.12*length(unique(coefs.drctn[padj <= 0.05, name])) +1 , width = 4)
}
