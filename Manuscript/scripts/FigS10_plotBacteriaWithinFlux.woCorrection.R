...
for(s.set in sim.sets){
    ...
    for(i in 1:length(phenotypes)){
        ...
        fl <- list.files(file.path(dat.dir,s.set), pattern = paste0("^withinFlux\\.", ph.type, "_coefs.csv"))
...
# some new columns and some filtering
dataset.flux <- dataset.flux[!is.na(EffSize),]
...
ggsave(plt.all,
       file = file.path("..","figures_raw","FigS10_withinFluxMicrobiom.woCorrection.pdf"),
       height = 2.5,
       width = 4)
