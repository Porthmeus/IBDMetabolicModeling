...
for(set in c("LMM.HBMayo.etiology","GLMM.Response.intervention","GLMM.Remission.intervention")){
    ...
    #    dotplot(hgt, x = "Count", showCategory = nrow(data.frame(hgt)))
        ...
        if(grepl("GLMM",set)){
            coefs.all.subs[,Estimate := sign(Estimate)*sqrt(abs(Estimate))]
            xlab = "sqrt(Estimate)"
            ttl = paste("Host", set.name)
        } else {
            xlab = "Estimate"
            ttl = paste("Host", "HBI/Mayo score")
        }
        # remove 0 and INF odds ratios from the list, because these are awkward to plot
        if(grepl("GLMM", set)){
           coefs.all.subs  <- coefs.all.subs[OR !=0 & OR != Inf,]
        }
        ...
        plt.sub.est.sig <- ggplot(coefs.plt, aes(x= Estimate, y = subsystem)) +
            ...
            theme_bw()
        #browser()
        ggsave(plt.sub.est.sig, file = file.path("..","figures_raw",paste0("Fig1",LETTERS[figNo],"_HostSubsystemPlots",set,".pdf")),
    ...
    write.csv(sig.subs.df, file = file.path(dat.dir, "EnrichedSubsystems.csv"), row.names = FALSE)
