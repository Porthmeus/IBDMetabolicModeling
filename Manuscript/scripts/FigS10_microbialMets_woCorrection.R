...
coefs_summary <- list()
figno<- 0
for(set in c("Response","Remission")){
    figno <- figno+1
    # coefficients
    ...
    # correct some inconsistencies in naming
    if("Coef" %in% colnames(ba.coefs.exp)){
        ba.coefs.exp[,coef :=Coef]
        ba.coefs.exp[,Coef := NULL]
    }
    ...
    pl <- ggplot(coefs.drctn[padj <= 0.05,],aes(x=Estimate, y = name, color = dataset, shape = by.microbiome)) +
        ...
        ggtitle(title)+
        theme_bw()
    ggsave(pl, file = paste0(file.path("..","figures_raw","/FigS10"),LETTERS[figno],"_MicrobialMetabolites.", set, ".woCorrection.pdf"), height = 0.12*length(unique(coefs.drctn[padj <= 0.05, name])) +1 , width = 4)
}
