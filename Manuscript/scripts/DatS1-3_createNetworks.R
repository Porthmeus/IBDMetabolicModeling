...
for(set in c("HBMayo","Response","Remission")){
    ...
    nodes.edges <- pruneNetwork(nodes = nodes, edges = edges.id)
    ...
    dg.pvals <- fread(file.path(dat.dir, paste(set,tst.cent,"significance.csv", sep = ".")))
    ...
    edges.final <- edges.id[,.(
                         ...
                         subsystem = subsystem,
                         #label = rxn.id,
                         color = mapColorToScale(sign(get(set.rxn)),cols =brewer.pal(name = "PiYG",n=9)[c(1,9)] ),
    ...
    vis.net <- visNetwork(nodes = nodes.final,
        ...
        visIgraphLayout(randomSeed = 346, layout = "layout_components") %>%
        visOptions(highlightNearest = list(enabled = TRUE, degree = 1, hover =FALSE, labelOnly = FALSE, hideColor = rgb(200,200,200,255*0.3,maxColorValue=255)),
        ...
        visEdges(width = 3)
    visSave(vis.net,
            file =file.path(res.dir, paste0("DatS",i,"_", set,".network.html")),
            selfcontained = TRUE
    )
}
