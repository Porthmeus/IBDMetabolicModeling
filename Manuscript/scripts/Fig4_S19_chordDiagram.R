...
for(st in c("Response","Remission","HBMayo")){
    ...
    for(tis in c("biopsy","blood")){
        ...
        pathdist <- distances(graph, weights = log(E(graph)$distance), mode = "out")
...
#        # check for most common metabolite along the shortest paths
#        for(i_n in 1:ncol(cmprs.pairs)){
#            nn_met <- cmprs.pairs[1,i_n]
#            nm_met <- cmprs.pairs[2,i_n]
#                shortest <- all_shortest_paths(graph,weights = log(E(graph)$distance), from =met.sel[[nn_met]], to = met.sel[[nm_met]])
#                mets_along_path <- names(unlist(shortest[["vpaths"]]))
#                mets_along_path <- mets_along_path[!(mets_along_path %in% unlist(met.sel[c(nn_met,nm_met)]))]
#                print(paste(nn_met, nm_met))
#                print(
#                    sort(table(mets_along_path))
#                    ) 
#        }
        ...
        if(nrow(dists.sel) > 2){
            ...
            grph <- graph_from_data_frame(dists.sel[,.(from,to,distance,con.sets,group.from,group.to)], directed = TRUE ,vertices = met.sel.dt[,..sel])
    ...
    #        grph.layout<- create_layout(grph,layout = "linear", circular = TRUE) 
            circle.plt <- ggraph(grph, layout = "linear", circular = TRUE) +
                ...
                guides("edge_alpha" = guide_edge_direction(title = "Path direction",arrow = F))
        circle.plt

        # safe a table of the actual shortest paths between the pairs of metabolites in the data
        # sort the index for the edges and nodes.metmic
        id_first <- which(colnames(edges) %in% c("from.id","to.id"))
        col_idx.edge <- c(colnames(edges)[id_first], colnames(edges)[-id_first])
        id_first <- grep("met.id", colnames(nodes))
        ...
        graph <- graph_from_data_frame(d = edges[,mget(col_idx.edge)],
                                       vertices = nodes[,mget(col_idx.node)])
        shrts_pths.rxn.all <- data.table()
        for(frm in dists.sel[,unique(from)]){#}

            from.met <- gsub("^Hub_|^Met_|^Mic_","",frm)
            tmet <- dists.sel[from ==frm,to]
            to.mets <- gsub("^Hub_|^Met_|^Mic_","",dists.sel[from ==frm,to])
            shrts_pths <-shortest_paths(graph, from = from.met, to = to.mets, algorithm = "Dijkstra", output = "both")[["epath"]]
            # get the reactions along the path

            shrts_pths.rxn <- data.table()
            for(pth.id in 1:length(shrts_pths)){
                pth.rxns <- c()
                for(dge in 1:length(shrts_pths[[pth.id]])){
                    pth.rxns <- c(pth.rxns, shrts_pths[[pth.id]][[dge]]$rxn.id)
                }
                shrts_pths.rxn <- rbind(shrts_pths.rxn,
                                        data.table(from = from.met,
                                                   to = to.mets[[pth.id]],
                                                   from.group = frm,
                                                   to.group = tmet[[pth.id]],
                                                   rxn = pth.rxns)
                                        )
            }
            shrts_pths.rxn.all <- rbind(shrts_pths.rxn.all,
                                        shrts_pths.rxn)
        }
        fwrite(shrts_pths.rxn.all, file.path(dat.dir, paste("shortPath_005quant",tis,st,"csv", sep = ".")))
    ...
    for(tis in c("biopsy","blood")){
        ...
        dist.plt <- ggplot(dists.sel, aes(x=con.sets, y = distance))+
            ...
            geom_text(data=stats,
                      #aes(label =label2))+
                      aes(label = paste0("p = ",formatC(padj, format = "e", digits = 2))),
                      angle = 90,
                      size = 5)+
...
ggsave(hbmayo.plt, file = file.path(res.dir, paste0("Fig4_pathwayPlots.HBMayo.pdf")),
           width = 14, height = 16)
response.plt <- plot_grid(boxplot.plts[[1]], circle.plts[[1]],boxplot.plts[[2]], circle.plts[[2]],
                        ncol = 2, rel_widths=c(1,5), labels = "AUTO")
ggsave(response.plt, file = file.path(res.dir, paste0("FigS19_pathwayPlots.Response.pdf")),
           width = 14, height = 16)
