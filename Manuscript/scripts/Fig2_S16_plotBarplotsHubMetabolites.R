...
sets <- rev(c("HBMayo","Response","Remission"))
...
mets.coefs[,set2 := factor(gsub("HBMayo","HBI/Mayo score", set), levels = c("HBI/Mayo score","Response","Remission"))]
mets.coefs[,met.name2 := gsub("Coenzyme A", "CoA",met.name)]
mets.coefs[,met.name2 := gsub("Nicotinamide Adenine Dinucleotide phosphate", "NADP",met.name2)]
mets.coefs[,met.name2 := gsub("Nicotinamide Adenine Dinucleotide", "NAD",met.name2)]
mets.coefs[,met.name2 := gsub("Flavin Adenine Dinucleotide", "FAD",met.name2)]
mets.coefs[,met.name2 := gsub("Adenosine Monophosphate", "AMP",met.name2)]
mets.coefs[,met.name2 := gsub("Adenosine Diphosphate", "ADP",met.name2)]
mets.coefs[,met.name2 := gsub("Adenosine Triphosphate", "ATP",met.name2)]
...
# plot a boxplot similar to the subsystem enrichment
mets.coefs[,Tissue := tools::toTitleCase(as.character(tissue))]
mets.coefs[,Tissue := relevel(factor(Tissue), "Biopsy")]

box.plt <- ggplot(mets.coefs[set2 == "HBI/Mayo score",], aes(x=Estimate, y = met.name2))+
    geom_vline(xintercept = 0, linetype = 2, color = "red")+
    geom_boxplot()+
    facet_grid(~Tissue)+
    theme_bw()+
    labs(y = element_blank(), x = "Estimate (reac. activity during inflam.)")
box.plt
...
met.nwk.cnt.melt[,UpDown := gsub(".*\\.(.*$)","\\1",variable)]
...
# create a hbmayo_only version for metabolites being products/substrates
met.nwk.cnt.melt.2 <- met.nwk.cnt.melt[,.(total = unique(total), count = sum(abs(count))), by = .(tissue, set, met.name,Metabolite,UpDown)]
met.nwk.cnt.melt.2[,rel_count := count/total]
met.nwk.cnt.melt.2[,`Regulation/Metabolite` := factor(paste(UpDown, Metabolite), levels = c("up substrate","up product","up both","down substrate","down product","down both"))]

bar.pltX <- ggplot(met.nwk.cnt.melt.2[set == "HBI/Mayo score",], aes(y= met.name, x = rel_count))+
    geom_bar(stat = "identity", aes(fill = `Regulation/Metabolite`), alpha = 0.7) + 
    scale_fill_brewer(palette="RdYlBu")+
    scale_color_manual(values=c("black","gray80"))+
    scale_x_continuous(breaks = c(0:5*2/10))+
    facet_grid(~tools::toTitleCase(as.character(tissue)), scale = "free")+
    theme_bw() +
    labs(x = "Proportion", y = element_blank())+
    theme(axis.text.y = element_blank())
bar.pltX

box.bar.plt <- plot_grid(box.plt, bar.pltX, ncol = 2, labels = c("C","D"))
ggsave(box.bar.plt, file = file.path("..","figures_raw","Fig2_plotHostMetabolites.pdf"),
       width = 11.6,
       height = 5)
...
for(st in unique(met.nwk.cnt.melt[,set])){
    bar2.plt.hbmayo <- ggplot(met.nwk.cnt.melt[set == st,], aes(y=rel.count, x = met.name))+
        ...
        theme(axis.text.x = element_text(angle = 60, hjust = 1, vjust = 1))
    bar2.plt.hbmayo
    ggsave(bar2.plt.hbmayo, file = paste0("../figures_raw/FigS16_MetabolitesBarplot.",gsub("/| ","",st)  ,".pdf"),
           width = 8, height = 5)
}
