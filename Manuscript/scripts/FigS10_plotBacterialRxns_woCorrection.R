...
phenotypes <- c(Response = "Response",
          Remission = "Remission")
...
for(s.set in sim.sets){
    ...
    for(i in 1:length(phenotypes)){
        ...
        fl <- list.files(file.path(dat.dir,s.set), pattern = paste0("^flux\\.",  ph.type,  "_coefs.csv"))
...
ggsave(rxn.plt,
       ...
       height = 10,
       width = 12,
       file = file.path("..","figures_raw","FigSXX_internalRxns.Microbiome.Estimates.woCorrection.pdf"))
...
ggsave(venn.plt,
       file = file.path("..","figures_raw", "FigS10_Microbiome.internalRxnsVenn.woCorrection.pdf"),
       width = 6.5, height = 3)
...
plt.dat.nrch <- merge(dataset.flux[,.(rxn, sim.set, phenotype, Estimate, padj, pathway)],rxn2nrch,
...
ggsave(plt.all,
       file = file.path("..","figures_raw","FigS10_EnrichedPathwaysMicrobeRxn.woCorrection.pdf"),
       height = x,
       width = 5)

write.csv(dat.final, row.names = FALSE,
          file = file.path("..","temp","EnrichedMicPathways.woCorrection.csv"))
