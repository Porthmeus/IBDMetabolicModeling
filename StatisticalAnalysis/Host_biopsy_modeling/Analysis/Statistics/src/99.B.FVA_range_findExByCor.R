...
FVA.range <- FVA.range[,!(colnames(FVA.range) %in% blckSmpls)]
meta <- meta[SeqID %in% colnames(FVA.range),]

# remove near zero variance reactions
near0var_idx <- apply(FVA.range,1,var) ==0
FVA.range <- FVA.range[!near0var_idx,]

# scale values
FVA.range <- t(scale(t(FVA.range)))

# get the exchange reactions
ex_rxns <- grep("^EX_", rownames(FVA.range), value =TRUE)
...
rxns.exsig.bool <- rownames(FVA.range) %in% rxns.exsig 

# calculate correlation coefficients
corre <- cor(t(FVA.range[rxns.exsig.bool,]))
...
gz <- gzfile(file.path(dr,"FVA.range_ExCorrelations.csv.gz"),"w")
