...
#for(reac in rxns){
    ...
    form <- as.formula(paste0("factor(Remission) ~ ",paste(sigs.FVA[rxn == reac, coef], collapse = "+"), "+ (1|PatientID)"))
...
names(mods) <- rxns
saveRDS(mods, file = "../results/FVA.GLMM.Remission.biomarker_models.RDS")
...
#        form_split <- form_split[-c(1,length(form_split))]
...
write.csv(stats, row.names=FALSE, file = "../results/FVA.GLMM.Remission.biomarker_coefs.csv")
...
i<-0
pdf("../results/FVA.GLMM.Remission.biomarker_sigDiagPlots.pdf")
for(rxn in rxns.sig){
