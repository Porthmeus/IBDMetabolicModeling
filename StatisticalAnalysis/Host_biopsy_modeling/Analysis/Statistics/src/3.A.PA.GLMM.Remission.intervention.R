...
#for(reac in rxns){
    dat <- rxnCount.meta[rn == reac & Time_seq != max(Time_seq),]
    mod <- tryCatch({glmer(data = dat,
          formula = factor(Remission) ~ HB_Mayo_impu + value + (1|PatientID),
          family = "binomial")},
                    error = function(e){
...
names(mods) <- rxns
saveRDS(mods, file = "../results/PA.GLMM.Remission.intervention_models.RDS")
#mods <- readRDS("../results/PA_GLMMsRemissionAll_models.RDS")
...
write.csv(stats, row.names=FALSE, file = "../results/PA.GLMM.Remission.intervention_coefs.csv")

# create diagnostic plots for the significant reactions
rxns.sig <- stats[padj < 0.05, rxn]
pdf("../results/PA.GLMM.Remission.intervention_sigDiag.pdf")
for(rxn in rxns.sig){
