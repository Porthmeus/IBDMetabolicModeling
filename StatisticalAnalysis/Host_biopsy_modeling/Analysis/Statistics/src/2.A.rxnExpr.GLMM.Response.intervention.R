...
#for(reac in rxns){
    dat <- rxnExpr.meta[rn == reac,]
    mod <- tryCatch({glmer(data = dat,
          formula = factor(Response) ~ HB_Mayo_impu + value + (1|PatientID),
          family = "binomial")},
                    error = function(e){
...
write.csv(stats, row.names=FALSE, file = "../results/rxnExpr.GLMM.Response.intervention_coefs.csv")
saveRDS(mods, file = "../results/rxnExpr.GLMM.Response.intervention_models.RDS")

# create diagnostic plots for the significant reactions
rxns.sig <- stats[padj < 0.05, rxn]
pdf("../results/rxnExpr.GLMM.Response.intervention_sigModsDiagnostics.pdf")
for(rxn in rxns.sig){
