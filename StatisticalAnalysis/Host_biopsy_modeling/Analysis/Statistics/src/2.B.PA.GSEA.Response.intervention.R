...
stats <- fread("../results/PA.GLMM.Response.intervention_coefs.csv")
...
## don't do it anymore, because it can cause errors downstream
#rmvd.rxns <- setdiff(rownames(rxnCount), stats.exp[,rxn])
...
# save the results
write.csv(stab, file = "../results/PA.GSEA.Response.intervention_GSEAStabilityAnalysis.csv")
write.csv(as.data.frame(estimates.all), file = "../results/PA.GSEA.Response.intervention_rankingVector.csv")
saveRDS(gsea, file = "../results/PA.GSEA.Response.intervention_GSEASubsystems.RDS")
