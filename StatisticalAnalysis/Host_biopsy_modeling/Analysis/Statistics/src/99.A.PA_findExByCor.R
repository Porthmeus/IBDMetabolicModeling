...
near0var_idx <- apply(rxnCount, 1, var) ==0
rxnCount <- rxnCount[!near0var_idx,]

# get the exchange reactions
ex_rxns <- grep("^EX_", rownames(rxnCount), value =TRUE)
...
rxns.exsig.bool <- rownames(rxnCount) %in% rxns.exsig 

# calculate correlation coefficients
corre <- cor(t(rxnCount[rxns.exsig.bool,]))
...
gz <- gzfile(file.path(dr, "PA_ExCorrelations.csv.gz"),"w")
