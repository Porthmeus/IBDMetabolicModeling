...
createMods <-function(rxn){
    ...
    mod <- tryCatch(glmer(data = dat.mod, factor(Response)  ~ Expression+(1|PatientID), family = "binomial"), 
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats.coef, file = "../results/rxnExpr.LMM.Response_coefs.csv")
...
#}
#pdf("../results/rxnExpr.LMM.Response_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
