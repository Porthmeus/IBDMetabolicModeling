...
for(i in 1:length(lowCor)){
        ...
        # if the cor.coef < 0.6 use both features in formula, otherwise use just the center
            ...
            form <- unique(c(form,  "factor(Response) ~ center + range + (1|PatientID)"))
        }
...
stats.all[padj < 0.05,]
write.csv(stats.all, file = "../results/FVA.LMM.Response_coefs.csv", row.names = FALSE)
