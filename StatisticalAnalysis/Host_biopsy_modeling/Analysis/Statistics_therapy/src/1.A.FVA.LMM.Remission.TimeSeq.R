...
for(i in 1:length(lowCor)){
        ...
        # if the cor.coef < 0.6 use both features in formula, otherwise use just the center
            ...
            form <- unique(c(form,  "range ~ Remission*Time_seq + (1|PatientID)"))
        }
...
stats.coef <- stats.coef[coef2 == "RemissionR:Time_seq",]
stats.coef[,padj := p.adjust(`Pr(>|t|)`, method = "BH")] 
# save the coefficients table
write.csv(stats.coef, file = "../results/FVA.LMM.Remission.TimeSeq_coefs.csv", row.names = FALSE)
