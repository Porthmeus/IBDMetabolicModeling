...
dat <- dat[Remission != "C" & Time_seq %in% c(0,14),]
dat.cast <- dcast(data = dat, PatientID + rn ~ paste0("d",Time_seq), value.var = "value")
dat.cast[,value.diff := d14-d0]
dat.cast <- merge(dat.cast, unique(meta[,.(PatientID, Remission)]), by ="PatientID")
...
createMods <-function(rxn){
    ...
    dat.mod <- dat.cast[rn == rxn,]
    mod <- tryCatch(glm(data = dat.mod, factor(Remission) ~ factor(value.diff), family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
    ...
    stat <- data.table(as.data.frame(coef(summary(mod)))["value.diff",],keep.rownames = TRUE)
...
write.csv(stats, file = "../results/PA.LMM.Remission.14d0dDiff_coefs.csv", row.names = FALSE)


## create diagnostic plots for the significant reactions
#rxns.sig <- stats[padj < 0.05, rxn]
#pdf("../results/PA.LMM.Remission.Baseline_sigModsDiagnostics.pdf")
#for(rxn in rxns.sig){
#    dat.mod <- dat[rn == rxn,]
