...
#for(reac in rxns){
    dat <- FVA[rn == reac,]
    form <- as.formula(paste0("factor(Response) ~ HB_Mayo_impu +",paste(sigs.FVA[rxn == reac, coef], collapse = "+"), "+ (1|PatientID)"))
...
names(mods) <- rxns
saveRDS(mods, file = "../results/FVA.GLMM.Response.intervention_models.RDS")
...
write.csv(stats, row.names=FALSE, file = "../results/FVA.GLMM.Response.intervention_coefs.csv")


# create diagnostic plots for the significant reactions
rxns.sig <- unique(stats[padj < 0.05, rxn])
pdf("../results/FVA.GLMM.Response.intervention_sigModsDiagnostics.pdf")
for(rxn in rxns.sig){
