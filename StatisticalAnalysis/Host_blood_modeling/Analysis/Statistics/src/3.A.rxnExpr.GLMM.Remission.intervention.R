...
#for(reac in rxns){
    dat <- rxnExpr.meta[rn == reac & Time_seq != max(Time_seq),]
...
names(mods) <- rxns
saveRDS(mods, file = "../results/rxnExpr.GLMM.Remission.intervention_models.RDS")
...
write.csv(stats, row.names=FALSE, file = "../results/rxnExpr.GLMM.Remission.intervention_coefs.csv")

# create diagnostic plots for the significant reactions
rxns.sig <- stats[padj < 0.05, rxn]
pdf("../results/rxnExpr.GLMM.Remission.intervention_sigDiagPlots.pdf")
for(rxn in rxns.sig){
