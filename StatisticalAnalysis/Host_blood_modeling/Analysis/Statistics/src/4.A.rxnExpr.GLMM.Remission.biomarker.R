...
#for(reac in rxns){
    ...
    mod <- tryCatch({glmer(data = dat,
          formula = factor(Remission) ~ value + (1|PatientID),
          family = "binomial")},
                    error = function(e){
...
write.csv(stats, row.names=FALSE, file = "../results/rxnExpr.GLMM.Remission.biomarker_coefs.csv")
saveRDS(mods, file = "../results/rxnExpr.GLMM.Remission.biomarker_models.RDS")
...
i<-0
pdf("../results/rxnExpr.GLMM.Remission.biomarker_sigDiagPlots.pdf")
for(rxn in rxns.sig){
