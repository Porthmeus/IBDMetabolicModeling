...
# get the clustered reactions
tbl.cluster <- fread(file = "../results/rxnExpr_DBSCAN_Cluster.csv")
...
names(mods) <- rxns
saveRDS(mods, file = "../results/rxnExpr.LMM.HBMayo.etiologyDisSpec_models.RDS")
...
# save the data
write.csv(stats.coef, file = "../results/rxnExpr.LMM.HBMayo.etiologyDisSpec_coefs.csv")
...
pdf("../results/rxnExpr.LMM.HBMayo.etiologyDisSpec_sigDiagPlots.pdf", width = 8, height= 5)
i <- 0 
for(rxn in rxns){
