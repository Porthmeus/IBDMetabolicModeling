...
dat <- dat[Response != "C" & Time_seq == 14,]
...
write.csv(stats, file = "../results/PA.LMM.Response.14d_coefs.csv", row.names = FALSE)


# create diagnostic plots for the significant reactions
#rxns.sig <- stats[padj < 0.05, rxn]
#pdf("../results/PA.LMM.Response.14d_sigModsDiagnostics.pdf")
#for(rxn in rxns.sig){
#    dat.mod <- dat[rn == rxn,]
