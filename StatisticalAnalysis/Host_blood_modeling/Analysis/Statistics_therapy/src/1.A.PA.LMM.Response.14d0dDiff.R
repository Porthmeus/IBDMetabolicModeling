...
dat <- dat[Response != "C" & Time_seq %in% c(0,14),]
dat.cast <- dcast(data = dat, PatientID + rn ~ paste0("d",Time_seq), value.var = "value")
dat.cast[,value.diff := d14-d0]
dat.cast <- merge(dat.cast, unique(meta[,.(PatientID, Response)]), by ="PatientID")
...
createMods <-function(rxn){
    ...
    mod <- tryCatch(glm(data = dat.mod, factor(Response) ~ factor(value.diff), family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats, file = "../results/PA.LMM.Response.14d0dDiff_coefs.csv", row.names = FALSE)


## create diagnostic plots for the significant reactions
#rxns.sig <- stats[padj < 0.05, rxn]
#pdf("../results/PA.LMM.Response.Baseline_sigModsDiagnostics.pdf")
#for(rxn in rxns.sig){
#    dat.mod <- dat[rn == rxn,]
