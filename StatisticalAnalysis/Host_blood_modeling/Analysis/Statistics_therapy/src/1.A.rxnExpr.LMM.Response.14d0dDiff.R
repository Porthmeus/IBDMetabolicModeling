...
rxnExpr.melt <- rxnExpr.melt[Response != "C" & Time_seq %in% c(0,14),]
...
rxnExpr.cast <- merge(rxnExpr.cast, unique(meta[,.(PatientID, Response)]), by = "PatientID")
...
createMods <-function(rxn){
    ...
    mod <- tryCatch(glm(data = dat.mod, factor(Response) ~ Expression.diff, family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats.coef, file = "../results/rxnExpr.LMM.Response.14d0dDiff_coefs.csv")
...
#pdf("../results/rxnExpr.LMM.Response.Baseline_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
#    mod <- lmer(data = rxnExpr.melt[rn == rxn,],Expression ~ Response*Time_seq+(1|PatientID))
