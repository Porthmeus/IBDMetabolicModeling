...
bar.plt <- ggplot(dat, aes(x=tested.condition, y = counts))+
    ...
    theme_bw()+
    labs(x=element_blank(), y = "sig. rxns", title = "Blood")+
...
bar.plt2 <- ggplot(bar.dat, aes(x=covariate, y = count))+
    ...
    theme_bw()+
    facet_grid(~outcome)+
    labs(x="covariate/subset/interaction", y = "sig. rxns", title = "Blood")+
    theme(axis.text.x= element_text(angle = 45, hjust = 1, vjust = 1))
...
#                  counts = unlist(sig.counts))
#dat2[,c("tested.condition", "data.layer") := list(gsub(".biomarker","",gsub(".*_(.*)","\\1",cnd)),
#                                                 gsub("^(.*)_.*","\\1",cnd))]
#
...
#oldCorrected <- fread(file.path(old.res.dr, "FVA.GLMM.Remission.biomarker_coefs.csv"))
