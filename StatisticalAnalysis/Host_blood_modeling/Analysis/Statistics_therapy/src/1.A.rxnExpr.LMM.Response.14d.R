...
rxnExpr.melt <- rxnExpr.melt[Response != "C" & Time_seq ==14,]
...
createMods <-function(rxn){
    ...
    mod <- tryCatch(glm(data = dat.mod, factor(Response) ~ Expression, family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats.coef, file = "../results/rxnExpr.LMM.Response.14d_coefs.csv")
...
#pdf("../results/rxnExpr.LMM.Response.14d_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
#    mod <- glmer(data = rxnExpr.melt[rn == rxn,],factor(Response) ~ Expression+(1|PatientID), family = "binomial")
