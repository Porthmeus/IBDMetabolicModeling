...
# remove controls
FVA <- FVA[Remission != "C" & Time_seq == 0,]
...
for(i in 1:length(lowCor)){
        ...
        # if the cor.coef < 0.6 use both features in formula, otherwise use just the center
            ...
            form <- unique(c(form,  "factor(Remission) ~ center + range"))
        }
...
stats.all[padj < 0.05,]
write.csv(stats.all, file = "../results/FVA.LMM.Remission.Baseline_coefs.csv", row.names = FALSE)
