...
# remove controls
FVA <- FVA[Remission != "C" & Time_seq %in% c(0,14),]
...
dat <- merge(dat, unique(meta[,.(PatientID, Remission)]), by = "PatientID")
...
for(i in 1:length(lowCor)){
        ...
        # if the cor.coef < 0.6 use both features in formula, otherwise use just the center
            ...
            form <- unique(c(form,  "factor(Remission) ~ center.diff + range.diff"))
        }
...
stats.all[padj < 0.05,]
write.csv(stats.all, file = "../results/FVA.LMM.Remission.14d0dDiff_coefs.csv", row.names = FALSE)
