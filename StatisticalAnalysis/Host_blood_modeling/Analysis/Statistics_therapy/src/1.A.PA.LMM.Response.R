...
createMods <-function(rxn){
    ...
    dat.mod <- dat[rn == rxn,]
    mod <- tryCatch(glmer(data = dat.mod, factor(Response) ~ value+(1|PatientID), family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats, file = "../results/PA.LMM.Response_coefs.csv", row.names = FALSE)
