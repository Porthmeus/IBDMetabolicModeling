...
rxnExpr.melt <- rxnExpr.melt[Response != "C",]
...
createMods <-function(rxn){
    ...
    mod <- tryCatch(lmer(data = dat.mod, Expression ~ Response*Time_seq+(1|PatientID)),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats.coef, file = "../results/rxnExpr.LMM.Response.TimeSeq_coefs.csv")
...
#pdf("../results/rxnExpr.LMM.Response.TimeSeq_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
