...
dat <- dat[Remission != "C" & Time_seq != max(Time_seq),]
...
createMods <-function(rxn){
    ...
    dat.mod <- dat[rn == rxn,]
    mod <- tryCatch(glmer(data = dat.mod, factor(Remission) ~ value+(1|PatientID), family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats, file = "../results/PA.LMM.Remission_coefs.csv", row.names = FALSE)


# create diagnostic plots for the significant reactions
#rxns.sig <- stats[padj < 0.05, rxn]
#pdf("../results/PA.LMM.Remission_sigModsDiagnostics.pdf")
#for(rxn in rxns.sig){
#    dat.mod <- dat[rn == rxn,]
