...
createMods <-function(rxn){
    ...
    dat.mod <- dat[rn == rxn,]
    mod <- tryCatch(glmer(data = dat.mod, value ~ Remission*Time_seq+(1|PatientID), family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats, file = "../results/PA.LMM.Remission.TimeSeq_coefs.csv", row.names = FALSE)
