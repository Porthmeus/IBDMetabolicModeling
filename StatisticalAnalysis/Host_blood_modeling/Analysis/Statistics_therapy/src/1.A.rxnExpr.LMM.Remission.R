...
rxnExpr.melt <- rxnExpr.melt[Remission != "C" & Time_seq != max(Time_seq),]
...
createMods <-function(rxn){
    ...
    mod <- tryCatch(glmer(data = dat.mod, factor(Remission)  ~ Expression+(1|PatientID), family = "binomial"), 
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats.coef, file = "../results/rxnExpr.LMM.Remission_coefs.csv")
...
#}
#pdf("../results/rxnExpr.LMM.Remission_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
