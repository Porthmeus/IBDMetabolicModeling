...
createMods <-function(rxn){
    ...
    dat.mod <- dat[rn == rxn,]
    mod <- tryCatch(glm(data = dat.mod, factor(Response) ~ value, family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    })
...
write.csv(stats, file = "../results/PA.LMM.Response.Baseline_coefs.csv", row.names = FALSE)
