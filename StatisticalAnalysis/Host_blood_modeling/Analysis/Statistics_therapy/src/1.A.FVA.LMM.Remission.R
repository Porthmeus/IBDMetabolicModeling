...
# remove controls
FVA <- FVA[Remission != "C" & Time_seq != max(Time_seq),]
...
for(i in 1:length(lowCor)){
        ...
        # if the cor.coef < 0.6 use both features in formula, otherwise use just the center
            ...
            form <- unique(c(form,  "factor(Remission) ~ center + range + (1|PatientID)"))
        }
...
createMods <- function(i){
    ...
    rxn <- forms[i, reac]
    dat <- FVA[rn == rxn,]
    mod <- tryCatch(glmer(data = dat, form, family = "binomial"),
                    error = function(e){
                        print(e)
                        return(NA)
                    }
    ...
    if(!is.na(mod)){
        form_split <- colnames(coef(mod)[[1]])[-(1)]
...
stats.all[padj < 0.05,]
write.csv(stats.all, file = "../results/FVA.LMM.Remission_coefs.csv", row.names = FALSE)
...
#pdf("../results/FVA.LMM.Remission.TimeSeq_sigDiagPlots.pdf", width = 8, height= 5)
#i <- 0 
#for(rxn in rxns){
