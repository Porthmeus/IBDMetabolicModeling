...
mods <- foreach(rxn = rxns) %dopar%{
    ...
    mod <- tryCatch({glmer(data = dat,
                formula = Response ~  source + seqtype + Age + outflux + (1|PatientID),
                family = "binomial")},
                    error = function(e) {
...
write.csv(file = "../results/outflux.Response_coefs.csv", stats.coef, row.names = FALSE)

# do the simple linear models as well
dats <- list()
dats[["d0"]] <- outfluxes.merged[Response != "C" & Time_seq.x == 0, .(Response, source, seqtype, Age, value = outflux, PatientID, ex.rxn)]
dats[["d14"]] <- outfluxes.merged[Response != "C" & Time_seq.x == 14, .(Response, source, seqtype, Age, value = outflux, PatientID, ex.rxn)]
# calculate difference between time points
dat.temp <- outfluxes.merged[Response != "C" & Time_seq.x %in% c(0, 14), .(Response, source, seqtype, Age, value = outflux, PatientID, ex.rxn, Time_seq = Time_seq.x)]
...
for(dat.n in names(dats)){
    ...
    fwrite(stats, file = paste0("../results/outflux.Response.",dat.n,"_coefs.csv"))
}
