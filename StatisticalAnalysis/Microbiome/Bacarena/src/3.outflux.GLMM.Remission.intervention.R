...
# load data sets
fluxes <- read.csv("../../../resources/future.eMed.bacarena_outflux.csv", row.names = 1)
# remove internal fluxes
sel <- grep("fluxes.avail",colnames(fluxes))
fluxes <- fluxes[,sel]
...
write.csv(file = "../results/outflux.GLMM.Remission.intervention_coefs.csv", stats.coef, row.names = FALSE)
...
pdf("../results/outflux.GLMM.Remission.intervention_sigDiagPlots.pdf", width = 8, height= 5)
i <- 0 
for(rxn in rxns){
