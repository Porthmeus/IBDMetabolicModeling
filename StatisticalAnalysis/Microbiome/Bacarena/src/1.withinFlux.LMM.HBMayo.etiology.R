...
# create clusters for the withinFluxes
## remove near zero variance
nzv <- nearZeroVar(withinFluxes)
withinFluxes <- withinFluxes[,-nzv]

## scale the values
withinFluxes <- scale(withinFluxes)
## create a correlation matrix
withinFluxes.cor <- cor(withinFluxes)
# save the sign of correlation for later
corr.sign <- melt(data.table(sign(withinFluxes.cor),
                                        keep.rownames = TRUE),
                             id.var = "rn",
                             variable.name = "rep.rxn",
                             value.name = "sign.cor")
colnames(corr.sign)[1] <- "rxn" 
withinFluxes.cor <- 1-sqrt(withinFluxes.cor^2)

set.seed(528) # change in your code
cluster <- dbscan(withinFluxes.cor, # the matrix with the correlation coefficients converted to distance
...
tbl.cluster <- data.table( cluster = cluster[["cluster"]],
                          ...
                          rxn = rownames(withinFluxes.cor))

## get a representative reaction for each cluster
tbl.cluster[cluster == 0 ,rep.rxn := rxn]
tbl.cluster[cluster != 0, rep.rxn := names(which.min(rowSums(withinFluxes.cor[rxn,rxn]))), by = cluster]

# add back the sign of the correlation
tbl.cluster <- merge(tbl.cluster, corr.sign)

write.csv(file = "../results/withinFlux_cluster.csv", tbl.cluster)
...
withinFluxes.melt <- merge(withinFluxes.melt, idconv, by.x ="SeqID", by.y = "sample", all.x = TRUE)
...
mods <- foreach(rxn = rxns) %dopar%{
    ...
    mod <- lmer(data = dat,
                formula = HB_Mayo_impu ~ source + seqtype + Age + withinFlux + (1|PatientID))
}

names(mods) <- rxns
saveRDS(mods, file = "../results/withinFlux.LMM.HBMayo.etiology_mods.RDS")
...
write.csv(file = "../results/withinFlux.LMM.HBMayo.etiology_coefs.csv", stats.coef, row.names = FALSE)


rxns <- stats.coef[padj <= 0.05, rxn]
if(length(rxns) > 0){
    pbar <- txtProgressBar(min = 0 , max = length(rxns), style = 3)
    pdf("../results/withinFlux.LMM.HBMayo.etiology_sigDiagPlots.pdf", width = 8, height= 5)
    i <- 0 
    for(rxn in rxns){
