...
sel <- grep("fluxes.inter",colnames(fluxes))
fluxes <- fluxes[,sel]
...
stats.coef[,cpd := gsub("^fluxes.inter_(cpd[0-9]*)","\\1", rxn)]
stats.coef <- merge(stats.coef, metabolites[,.(id, name)], by.x = "cpd", by.y= "id", all.x = TRUE)

write.csv(file = "../results/withinFlux.GLMM.Remission.intervention_coefs.csv", stats.coef, row.names = FALSE)
