...
rxnsAnno <- fread("https://raw.githubusercontent.com/jotech/gapseq/master/dat/seed_reactions_corrected.tsv")
...
write.csv(file = "../results/flux_cluster.csv", tbl.cluster)
...
fluxes.melt <- merge(fluxes.melt, idconv, by.x ="SeqID", by.y = "sample", all.x = TRUE)
...
names(mods) <- rxns
saveRDS(mods, file = "../results/flux.LMM.HBMayo.etiology_mods.RDS")
# mods <- readRDS("../results/flux.LMM.HBMayo.etiology_mods.RDS")
...
write.csv(file = "../results/flux.LMM.HBMayo.etiology_coefs.csv", stats.coef, row.names = FALSE)


rxns <- stats.coef[padj <= 0.05, rxn]
if(length(rxns) > 0){
    pbar <- txtProgressBar(min = 0 , max = length(rxns), style = 3)
    pdf("../results/flux.LMM.HBMayo.etiology_sigDiagPlots.pdf", width = 8, height= 5)
    i <- 0 
    for(rxn in rxns){
