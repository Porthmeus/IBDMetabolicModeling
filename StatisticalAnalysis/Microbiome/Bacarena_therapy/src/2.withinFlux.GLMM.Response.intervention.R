...
mods <- foreach(rxn = rxns) %dopar%{
    ...
    mod <- tryCatch({glmer(data = dat,
                formula = Response ~ source + seqtype + Age + withinFlux + (1|PatientID),
                family = "binomial")},
                    error = function(e) {
...
write.csv(file = "../results/withinFlux.Response_coefs.csv", stats.coef, row.names = FALSE)


# do the simple linear models as well
dats <- list()
dats[["d0"]] <- withinFluxes.merged[Response != "C" & Time_seq.x == 0, .(Response, source, seqtype, Age, value = withinFlux, PatientID, ex.rxn)]
dats[["d14"]] <- withinFluxes.merged[Response != "C" & Time_seq.x == 14, .(Response, source, seqtype, Age, value = withinFlux, PatientID, ex.rxn)]
# calculate difference between time points
dat.temp <- withinFluxes.merged[Response != "C" & Time_seq.x %in% c(0, 14), .(Response, source, seqtype, Age, value = withinFlux, PatientID, ex.rxn, Time_seq = Time_seq.x)]
dat.mat <- dcast(data=dat.temp, Age+Response+source+seqtype+PatientID+ex.rxn~paste0("d",Time_seq), value.var = "value")
...
for(dat.n in names(dats)){
    ...
    fwrite(stats, file = paste0("../results/withinFlux.Response.",dat.n,"_coefs.csv"))
}
