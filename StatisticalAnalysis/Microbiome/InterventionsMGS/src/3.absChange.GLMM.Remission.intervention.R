...
#        mods <- list(); for(met in mets){
            dat <- changes[intervention == intv & metabolite.name == met & Remission != "C" & Time_seq.x != maxT,]
            mod <- tryCatch({m <- glmer(data = dat,
                        formula = Remission ~ source + seqtype + Age + flux.change + (1|PatientID),
                        family = "binomial")
                            },
                            error = function(e) {
...
saveRDS(models.all, file = "../results/fluxChange.GLMM.Remission.intervention_mods.RDS")
...
gz <- gzfile("../results/fluxChange.GLMM.Remission.intervention_coefs.csv.gz","w")
...
if(nrow(vals) > 0){
    ...
    pdf("../results/absChange.GLMM.Remission.intervention_sigDiagPlots.pdf", width = 8, height= 5)
